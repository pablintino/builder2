...
def __install_components(components, target_dir, installation_summary):
    for component_key, component_config in components.items():
        with di.Container.tool_installers[type(component_config)](component_key, component_config,
                                                                  target_dir) as installer:
...
def __install(args,
    ...
    try:
        ...
        __install_system_packages(toolchain_metadata.system_packages, installation_summary, package_manager)
        __install_components(toolchain_metadata.components, target_dir, installation_summary)

        # Add conan profiles env vars and all component ones
        installation_summary.add_environment_variables(
            conan_manager.create_profiles_from_installation(installation_summary, target_dir))
