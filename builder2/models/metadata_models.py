...
class BaseComponentSchema(Schema):
    ...
    add_to_path = fields.Boolean(data_key='add-to-path', required=False, load_default=False)
    expected_hash = fields.Str(data_key='expected-hash', required=False, load_default=None)
    group = fields.Str(required=False, load_default=None)
    required_packages = fields.List(fields.String, data_key='required-packages', load_default=[])
...
class CompilerBuildSchema(BaseComponentSchema):
    conan_profile = fields.Boolean(data_key='conan-profile', required=False, load_default=False)
    config_opts = fields.List(fields.String, data_key='config-opts', required=False, load_default=[])


class GccComponentSchema(CompilerBuildSchema):
    languages = fields.List(fields.String, required=True)
    suffix_version = fields.Boolean(data_key='suffix-version', required=False, load_default=False)
...
class CppCheckComponentSchema(BaseComponentSchema):
    compile_rules = fields.Boolean(data_key='compile-rules', required=False, load_default=True)
...
class ToolchainComponentSchema(OneOfSchema):
    type_schemas = {'gcc-build': GccComponentSchema, 'clang-build': ClangComponentSchema,
                    'cppcheck-build': CppCheckComponentSchema, 'valgrind-build': ValgrindComponentSchema,
                    'download-only-compiler': DownloadOnlyCompilerComponentSchema,
                    'download-only': DownloadOnlyComponentSchema, 'source-build': SourceBuildSchema,
                    'cmake-build': CmakeBuildComponentSchema, 'jdk': JdkComponentSchema, 'maven': MavenComponentSchema}
...
class ToolchainMetadataSchema(Schema):
    components = fields.Dict(keys=fields.Str(), values=fields.Nested(ToolchainComponentSchema))
