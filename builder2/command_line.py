...
class LogPipe(threading.Thread):
    ...
    def __init__(self, logger):
        ...
        self.start()
        self.output = ''
        self._logger = logger if logger else logging.root
...
def run_process(command_list, cwd=None, timeout=180, shell=False):
    working_dir = os.getcwd() if not cwd else cwd
    start_time = time.time()
    try:
        with LogPipe(__logger) as pipe:
        ...
        __logger.error("Failed to execute [%s]. Exit code non-zero.", command_list)
        raise
