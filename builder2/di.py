...
from models.metadata_models import BaseComponentConfiguration, GccBuildConfiguration, CmakeBuildConfiguration, \
    DownloadOnlyConfiguration, CppCheckBuildConfiguration, SourceBuildConfiguration, ClangBuildConfiguration, \
    ValgrindBuildConfiguration, DownloadOnlyCompilerConfiguration, JdkConfiguration, MavenConfiguration
from package_manager import PackageManager
from tools.tool_installers import JdkInstaller, ToolInstaller, MavenInstaller, DownloadOnlyCompilerInstaller, \
    ValgrindSourcesInstaller, ClangSourcesInstaller, ToolSourceInstaller, CppCheckSourcesInstaller, \
    DownloadOnlySourcesInstaller, CMakeSourcesInstaller, GccSourcesInstaller
...
class Container(containers.DeclarativeContainer):
    ...
    command_runner = providers.Singleton(
        CommandRunner
    )
    ...
    cryptographic_provider = providers.Singleton(
    ...
    conan_manager = providers.Singleton(
        ConanManager,
        file_manager,
        command_runner
    )

    java_tools = providers.Singleton(
        JavaTools,
        file_manager,
        command_runner
    )
    ...
    maven_installer_factory = providers.Factory(
        MavenInstaller,
        file_manager=file_manager, cryptographic_provider=cryptographic_provider,
        command_runner=command_runner, package_manager=package_manager,
        core_count=config.core_count, time_multiplier=config.timout_mult
    )

    jdk_installer_factory = providers.Factory(
        JdkInstaller,
        file_manager=file_manager, cryptographic_provider=cryptographic_provider,
        command_runner=command_runner, package_manager=package_manager, java_tools=java_tools,
        core_count=config.core_count, time_multiplier=config.timout_mult)

    gcc_sources_installer_factory = providers.Factory(
        GccSourcesInstaller,
        file_manager=file_manager, cryptographic_provider=cryptographic_provider,
        command_runner=command_runner, package_manager=package_manager, compilers_support=compilers_support,
        core_count=config.core_count, time_multiplier=config.timout_mult)
