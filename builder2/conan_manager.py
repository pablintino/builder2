...
def __prepare_gcc_profile_file(
    ...
    gcc_version_output = builder2.command_line.run_process([gcc_path, "-v"])
...
def add_profiles_to_component(
    ...
    is_clang = isinstance(component_installation.configuration, ClangBuildConfiguration)
    ...
    if (is_gcc or is_clang) and component_installation.configuration.conan_profile:
        ...
        os.makedirs(conan_profiles_path, exist_ok=True)

        for release_type in CONAN_PROFILE_TYPES:
            config_parser = __prepare_common_profile_file(
                component_installation, release_type
            )
            if is_clang:
                config_parser = __prepare_clang_profile_file(
                    component_installation, config_parser
                )
            else:
                config_parser = __prepare_gcc_profile_file(
                    component_installation, config_parser
                )
            ...
            component_installation.conan_profiles[release_type.lower()] = profile_path
