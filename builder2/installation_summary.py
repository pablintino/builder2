...
class InstallationSummary:
    ...
    def __init__(self, installation_path=None, summary=None):
        ...
        if summary:
            ...
            self.__installation_path = summary['installation-path']
            self.__system_packages = summary.get('system-packages', [])
            for key, data in summary.get('components', {}).items():
                self.__components[key] = ComponentInstallation(**data)
            self.__environment_vars = summary.get('environment', {}).get('variables', {})
    ...
    def from_path(cls, path):
        ...
        with open(summary_path) as f:
            return cls(summary=json.load(f))
    ...
    def save(self, target_dir):
        ...
        inventory = {
            "installation-path": self.__installation_path,
            "components": self.__components,
            "system-packages": self.__system_packages,
            "environment": {
    ...
    def add_component(self, tool_summary: ComponentInstallation):
        self.__components[tool_summary.key] = tool_summary
    ...
    def get_components_by_type(self, component_type) -> [ComponentInstallation]:
        return [comp_installation for comp_installation in self.__components.values() if
                comp_installation.type == component_type]
