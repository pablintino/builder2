...
class JavaTools:
    ...
    def get_jdk_wellknown_paths(self, target_dir):
        ...
        if bin_dir.exists() and bin_dir.is_dir():
            ...
            self.__add_wellknown_if_exists(
                bin_dir, EXEC_NAME_JAVA_COMPILER, wellknown_paths
            )
            self.__add_wellknown_if_exists(bin_dir, EXEC_NAME_JAVA_DOC, wellknown_paths)
            self.__add_wellknown_if_exists(
                bin_dir, EXEC_NAME_JAVA_KEYTOOL, wellknown_paths
            )
    ...
    def get_from_manifest_var(
        self, maven_jar_path, var_name, ignore_failure=False, default=None
    ):
